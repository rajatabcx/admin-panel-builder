...
import { determineRelationType } from '@/lib/utils';
import { ActionResponse, Catalog, Relation } from '@/lib/types';
import { ResponseType } from '@/lib/constants';
import { getDbUrl } from './project';
import { currentUser } from './user';
import { createClient } from '@/lib/supabase/server';

export async function specificTableSchemaRelation(
  id: string,
  schema: string,
  tables: string[]
...
export const catalogExists = async (
  projectId: string
...
export const catalog = async (
  projectId: string
): Promise<ActionResponse & { data: Catalog | null }> => {
  const user = await currentUser();
  if (!user) {
    return {
      type: ResponseType.ERROR,
